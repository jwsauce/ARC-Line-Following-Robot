...
#define SPIN_360_MS   500      // ms for one 360° spin at SPIN_SPEED
...
// Clockwise: left fwd + right rev (viewed from above)
...
// Ramp down from fromSpeed → 0 over durationMs
...
void loop() {
    ...
    // ── 2. Slowly stop in 2s ─────────────────────────
    Serial.println("2. Slow STOP (forward) over 2s");
    slowStop(DRIVE_SPEED, true, 2000);

    // ── 3. Coast for 2s ──────────────────────────────
    Serial.println("3. COAST 2s");
    coast();
    delay(2000);
    ...
    // ── 5. Slowly stop in 2s ─────────────────────────
    Serial.println("5. Slow STOP (reverse) over 2s");
    slowStop(DRIVE_SPEED, false, 2000);

    // ── 6. Coast for 2s ──────────────────────────────
    Serial.println("6. COAST 2s");
    coast();
    delay(2000);
